...
namespace generator
{
    class Randomizer {
    public:
        // std::minstd_rand?  checkni mt19937 v codeblocks s std::random_device device; engine.seed(device());
        std::mt19937 engine;
        Randomizer()
        {
           ...
           // int maxS = config.configuration["max_width"] * config.configuration["max_height"];
            ...
            for (int i = 0; i < rowSize; i++)
            {
                for (int j = 0; j < rowSize; j++)
                {
                    ...
                    // second point, only y changed
                    int maxH = std::min(areaY - tmpPoint.second, config.configuration["max_height"]);
                    tmpPoint.second += this->randomizer.getRandomCoor(maxH, config.configuration["min_height"]);
                    tmpPolygon.push_back(tmpPoint);
                    // third point, only x changed
                    int maxW = std::min(areaX - tmpPoint.first, config.configuration["max_width"]);
